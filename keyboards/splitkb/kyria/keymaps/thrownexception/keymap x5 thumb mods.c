...
const key_override_t **key_overrides = (const key_override_t *[]) {
    &shift_comma_parenthesis_open, &shift_dot_parenthesis_close, &shift_quote_colon, &shift_comma_dash, &shift_dot_colon, &shift_zero_minus, NULL
};
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    releaseBeforePress = row % 4 == 3 || col < 2;
    ...
    if (layer_enabled(LAYER_NAV_BAR) && ((pressed && releaseBeforePress) || (!pressed && releaseAfterPress))) {
        layer_off(LAYER_NAV_BAR);
    }
    if (layer_enabled(LAYER_MODS) && pressed && (row % 4 != 3)) { // Not on the thumb keys
