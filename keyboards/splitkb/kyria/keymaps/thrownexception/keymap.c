...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [LAYER_CLMKDH] = LAYOUT(
        ...
        XXXXXXX, KC_Z, KC_X, KC_C, KC_D, KC_V, KC_LGUI, LGUI(KC_L), XXXXXXX, XXXXXXX, KC_K, KC_H, KC_COMMA, KC_DOT, KC_ENTER, XXXXXXX,
    ...
    [LAYER_GAME] = LAYOUT(
        ...
        XXXXXXX, XXXXXXX, CC_LTG_SYM, KC_SPACE, XXXXXXX, XXXXXXX, KC_SPACE, KC_ESC, XXXXXXX, XXXXXXX
    ),
    [LAYER_NAV_BAR] = LAYOUT(
    ...
    [LAYER_MODS] = LAYOUT(
        ...
        _______, _______, _______, OSM(MOD_LALT), _______, _______, OSM(MOD_LCTL), OSM(MOD_LSFT), _______, _______
    ),
    [LAYER_SYMBOLS] = LAYOUT(
        _______, _______, LSFT(KC_BACKSLASH), LSFT(KC_7), LSFT(KC_GRAVE), LSFT(KC_6), KC_PSCR, LSFT(KC_MINUS), KC_SLASH, KC_BACKSLASH, LSFT(KC_SLASH), _______,
        _______, _______, LSFT(KC_8), KC_SEMICOLON, LSFT(KC_QUOTE), LSFT(KC_4), LSFT(KC_2), KC_MINUS, LSFT(KC_LEFT_BRACKET), LSFT(KC_COMMA), KC_LEFT_BRACKET, _______,
        _______, _______, LSFT(KC_3), LSFT(KC_EQUAL), KC_GRAVE, OSM(MOD_LGUI), _______, _______, _______, _______, LSFT(KC_1), KC_EQUAL, LSFT(KC_RIGHT_BRACKET), LSFT(KC_DOT), KC_RIGHT_BRACKET, _______,
        _______, _______, _______, _______, _______, _______, _______, _______, CC_TOGGLE_BASE, _______
    ),
    [LAYER_SET] = LAYOUT(
    ...
    [LAYER_NAV] = LAYOUT(
        _______, KC_DEL, KC_HOME, KC_UP, KC_END, LCA(KC_5), KC_LALT, KC_7, KC_8, KC_9, KC_DOT, _______,
        _______, KC_TAB, KC_LEFT, KC_DOWN, KC_RIGHT, LSFT(KC_4), KC_LCTL, KC_4, KC_5, KC_6, KC_0, _______,
        _______, KC_ESC, LCTL(KC_X), LCTL(KC_C), LCTL(KC_V), LSFT(KC_5), XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, KC_LSFT, KC_1, KC_2, KC_3, KC_ENTER, _______,
        _______, _______, _______, _______, _______, _______, _______, _______, _______, _______
    )
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    releaseBeforePress = row % 4 == 3 || (row < 3 && col == 6) || col < 2;
    if (layer_enabled(LAYER_SYMBOLS) && keycode != CC_LTG_SYM && pressed == releaseBeforePress) {
        layer_off(LAYER_SYMBOLS);
    }
    releaseBeforePress = col != 6 || row >= 3; // Not in the left outer column
...
bool oled_task_user(void) {
    if (is_keyboard_master()) {
        ...
        oled_advance_page(true);
        oled_write_P(PSTR("NAV "), layer_enabled(LAYER_NAV_BAR));
        ...
        oled_write_P(PSTR(" MOD"), layer_enabled(LAYER_MODS));
        oled_advance_page(true);
        oled_advance_page(true);
        oled_write_P(PSTR("     "), false);
